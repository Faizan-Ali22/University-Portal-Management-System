...
// === Startup DB connectivity check (fail fast with clear log) ===
var logger = app.Services.GetRequiredService<ILogger<Program>>();
var connectionString = config.GetConnectionString("DefaultConnection");
if (string.IsNullOrWhiteSpace(connectionString))
{
    logger.LogCritical("Connection string 'DefaultConnection' is missing or empty in configuration.");
    throw new InvalidOperationException("Missing 'DefaultConnection' connection string.");
}

try
{
    await using var testConn = new SqlConnection(connectionString);
    await testConn.OpenAsync();
    logger.LogInformation("Successfully opened database connection for startup check.");
    await testConn.CloseAsync();
}
catch (Exception ex)
{
    logger.LogCritical(ex, "Unable to open database connection using connection string from 'DefaultConnection'. Check SQL Server instance name, that the SQL service is running, TCP/IP enabled, and firewall rules. Connection string: '{Conn}'", connectionString);
    // Fail fast so the developer sees a clear startup error
    throw;
}
// =============================================================
